        ...
        <div className="space-y-1">
          {categories.map((cat: Category) => (
            ...
            >
              ...
              />
              <span className="text-sm text-ink-2 select-none">{cat.name}</span>
            </label>
          ))}
          {categories.length === 0 && (
    ...
    <div className="space-y-5">
      <form onSubmit={handleSubmit} className="flex gap-2">
        <input
          ...
          className="flex-1 px-3 py-2 bg-bg border border-line rounded-md text-sm text-ink placeholder:text-ink-3 focus-input transition-all"
          required
        />
        <button
          type="submit"
          className="px-3.5 py-2 text-xs font-medium bg-accent text-white rounded-md hover:opacity-90 transition-opacity whitespace-nowrap"
        >
